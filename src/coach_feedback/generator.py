...
def generate_feedback(gi: GenerationInput) -> FeedbackOutput:
    ...
    if step == StepEnum.LINK_PRAISE_TO_STUDENT_LEARNING:
        ...
        improvement = "계획에 관찰 지표 1개(예: 손들기 수, 근거 발화 수)를 추가해 다음 수업에서 비교하세요."
    ...
    return FeedbackOutput(
        step_focus=step, praise=praise, improvement=improvement, why_it_matters=why,
        evidence_quote=quotes, student_learning_link=student_link, next_step=next_step, confidence=0.75
    )
