...
import asyncio
from ..schema import StepEnum, TranscriptChunk, GenerationInput
    ...
    try:
        while True:
            ...
            if isinstance(msg, dict) and msg.get('type') == 'RequestFeedback':
                ...
                await websocket.send_text(json.dumps({"type": "Ack", "receivedType": msg.get("type")}))
    ...
    async def run():
        try:
            if mode == "cloud":
                ...
                if audio_ref:
                    fb = run_pipeline_on_audio(audio_ref, preferred_step=StepEnum(force_step) if force_step else None)
                    payload = {"session_id": session_id, "step_focus": int(fb.step_focus), "feedback": fb.model_dump()}
                else:
                    # fallback: synthesize from sample transcript through generator
                    chunks = [TranscriptChunk(id="t1", speaker="teacher", text="지난 시간에 질문 뒤 2초를 기다리니 더 많은 학생이 손을 들었어요.")]
                    from ..schema import GenerationInput
                    gi = GenerationInput(transcript_chunks=chunks, step_focus=StepEnum.LINK_PRAISE_TO_STUDENT_LEARNING, language=language)
