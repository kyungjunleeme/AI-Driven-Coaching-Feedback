...
def audio_to_transcript_chunks(audio_path: str) -> List[TranscriptChunk]:
    ...
    chunks = [TranscriptChunk(id=f"seg{i}", speaker="teacher", text=s["text"]) for i, s in enumerate(segments, start=1)]
    if not chunks and full_text:
        chunks = [TranscriptChunk(id="t1", speaker="teacher", text=full_text)]
    return chunks

def run_pipeline_on_audio(audio_path: str, preferred_step: Optional[StepEnum] = None) -> FeedbackOutput:
    ...
    if top_step is None:
        ...
        for lab in cl_out.labels:
            if lab.step_ids and lab.confidence > max_conf:
                top_step = lab.step_ids[0]; max_conf = lab.confidence
        if top_step is None:
