...
def build_step_classifier_prompt(chunk_text: str, step_id: int) -> str:
    return render_with_jinja2('step_classifier.j2', step_name=STEP_NAMES[step_id], step_id=step_id, chunk_text=chunk_text)

def build_feedback_prompt(transcript: List[Dict[str, Any]], step_focus: int, language: str = "ko") -> str:
    return render_with_jinja2('feedback_generator.j2', transcript_json=json.dumps(transcript, ensure_ascii=False), step_focus=step_focus, language=language)

def _invoke_claude(prompt: str, system: Optional[str] = None, max_tokens: int = 400, temperature: float = 0.2) -> str:
    ...
    resp = runtime.invoke_model(modelId=model_id, body=json.dumps(body).encode("utf-8"), accept="application/json", contentType="application/json")
...
def classify_chunk_parallel_scores_concurrent(chunk_text: str, step_ids: List[int] = list(range(1,13))) -> Dict[int, float]:
    def scorer(step_id: int) -> float:
        prompt = build_step_classifier_prompt(chunk_text, step_id)
        out = _invoke_claude(prompt, system="Classify coaching step", max_tokens=200, temperature=0.0)
        try: data = json.loads(out); return float(data.get("score", 0.0))
        except Exception: return 0.0
    return parallel_scores_per_step(step_ids, scorer=scorer, max_workers=min(12, len(step_ids)))

def generate_feedback_cloud(transcript: List[Dict[str, Any]], step_focus: int, language: str = "ko") -> Dict[str, Any]:
    prompt = build_feedback_prompt(transcript, step_focus, language=language)
    out = _invoke_claude(prompt, system="Generate instructional coaching feedback", max_tokens=600, temperature=0.2)
    try:
        data = json.loads(out)
    except Exception:
        data = {
...
def _invoke_claude_cfg(prompt: str, system: Optional[str], max_tokens: int, temperature: float, timeout_s: int):
    ...
    if system:
        messages.append({"role": "system", "content": [{"type": "text", "text": system}]})
    messages.append({"role": "user", "content": [{"type": "text", "text": prompt}]})
    body = {"anthropic_version": "bedrock-2023-05-31", "max_tokens": max_tokens, "temperature": temperature, "messages": messages}
    resp = rt.invoke_model(modelId=model_id, body=json.dumps(body).encode("utf-8"), accept="application/json", contentType="application/json")
    payload = json.loads(resp["body"].read().decode("utf-8"))
    parts = payload.get("content", [])
    text = ""
    for p in parts:
...
def score_chunks_steps(texts: list[str], step_ids: list[int] = list(range(1,13)), max_workers: int = 24, timeout_s: int = 20, retries: int = 2, calls_per_sec: float = 8.0, initial_stagger_ms: int = 50) -> list[dict[int, float]]:
    ...
    def task(ci: int, sid: int):
        ...
        def call():
            limiter.acquire()
            out = _invoke_claude_cfg(prompt, system="Classify coaching step", max_tokens=200, temperature=0.0, timeout_s=timeout_s)
            try:
                data = json.loads(out)
