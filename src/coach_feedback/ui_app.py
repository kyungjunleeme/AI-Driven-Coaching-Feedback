...
def _load_sample() -> List[TranscriptChunk]:
    sample = [
        ...
        {"id": "t2", "speaker": "coach", "text": "좋아요. 다음에도 일관되게 적용해볼까요?"},
        {
            "id": "t3",
            "speaker": "teacher",
            "text": "네. 도입 질문마다 체크리스트로 표시해 보겠습니다.",
        },
    ]
...
def render_ui():
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            uploaded = st.file_uploader(
                "Upload transcript JSON (list of {id,speaker,text,ts?})", type=["json"]
            )
        ...
        with col2:
            ...
            if st.button("Generate"):
                if not chunks:
                    chunks = _load_sample()
                gi = GenerationInput(
                    transcript_chunks=chunks, step_focus=StepEnum(step_id), language=lang
                )
    ...
    with tabs[1]:
        ...
        chosen = st.selectbox(
            "Primary step (optional)", options=["auto"] + [s.value for s in StepEnum]
        )
        if st.button("Run voice pipeline"):
            if au is None:
                st.warning("Please upload an audio file.")
            else:
                fd, tmp = tempfile.mkstemp(suffix="-" + au.name)
                os.close(fd)
                with open(tmp, "wb") as f:
                    f.write(au.getbuffer())
                ...
                if st.button("Synthesize TTS of feedback"):
                    ...
                    summary = (
                        f"칭찬: {fb.praise}. 개선: {fb.improvement}. 다음 단계: {fb.next_step}."
                    )
    ...
    with tabs[2]:
        ...
        au_cloud = st.file_uploader(
            "Audio for cloud pipeline (.wav/.mp3)",
            type=["wav", "mp3", "m4a", "flac", "ogg"],
            key="au_cloud",
        )
        force_step = st.selectbox(
            "Force step (optional)", options=[None] + [s.value for s in StepEnum]
        )
        ...
        if st.button("Run Cloud pipeline (ASR → Bedrock → S3/DDB)"):
            if au_cloud is None:
                ...
                fd, tmp = tempfile.mkstemp(suffix="-" + au_cloud.name)
                os.close(fd)
                with open(tmp, "wb") as f:
                    f.write(au_cloud.getbuffer())
...
<h1>AsyncAPI spec</h1><pre>{html.escape(raw)}</pre></html>"""
            ...
            st.download_button(
                "Download static HTML", data=html_doc, file_name="asyncapi.html", mime="text/html"
            )
        except Exception as e:
